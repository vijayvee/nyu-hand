...
class ImageJitterer(object):
    ...
    def transformImg(self, img, t):
        imgT = img.transform((int(img.size[0] * t[3]), int(img.size[1] * t[3])), Image.EXTENT,
                             (0, 0, img.size[0], img.size[1]), Image.BILINEAR)
    ...
    def getJitteredImgs(self, img, num, maxRot=(-5.0, 5.0), maxTranslate=(-2.0, 2.0), maxScale=(-0.1, 0.1),
        ...
        for i in range(len(tMats)):
            ...
            if augmentColor:
                ...
                # add noise
                im = numpy.asarray(imgT).astype('int') + numpy.rint(
                    self.rng.normal(0, 4, numpy.asarray(imgT).shape)).astype('int')
